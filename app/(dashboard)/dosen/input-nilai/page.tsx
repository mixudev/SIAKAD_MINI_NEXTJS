...
import { BookOpen, Loader2, Presentation, ClipboardPenLine, CheckCircle2 } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {data.map((kelas: any) => (
            <Card key={kelas.id} className="bg-white border border-[#ececee] rounded-[8px] shadow-[rgba(0,0,0,0.04)_0px_4px_12px_0px] p-4">
              ...
              <Link href={`/dosen/input-nilai/${kelas.id}`}>
                <Button size="sm" className="h-7 text-[10px] font-semibold bg-[#09090b] text-white rounded-[6px] px-2.5 hover:bg-[#18181b] w-full">
                  <ClipboardPenLine className="size-3 mr-1" />
                  Input Nilai
                </Button>
              </Link>
            </Card>
